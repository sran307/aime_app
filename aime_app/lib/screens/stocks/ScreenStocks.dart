...
class _ScreenStocksState extends State<ScreenStocks> {
  Future<List<dynamic>>? screenStocks; // Future for fetching todo data

  @override
  void initState() {
    super.initState();
    // Call the function to fetch todo data
    screenStocks = fetchScreenedStock();
  }

  Future<List<dynamic>> fetchScreenedStock() async {
    dynamic screenStocks = await GetData(screener);
    int status = screenStocks.statusCode;
    if ([200, 201, 204].contains(status)) {
      // Parse the screenStocks JSON data
      final jsonData = jsonDecode(screenStocks.body);
      decryptor decode = decryptor();
      Map<String, dynamic> data = await decode.decrypt(jsonData['data']);
      return data['screenedStocks']; // Return the todo data
    } else {
      ApiResponse apiResponse = ApiResponse();
      await apiResponse.errorData(context, screenStocks);
      return [];
    }
  }
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: RefreshIndicator(
                          onRefresh: fetchScreenedStock,
                          child: SingleChildScrollView(
                            child: FutureBuilder<List<dynamic>>(
                              future:
                                  screenStocks, // Set the future to fetch todo data
